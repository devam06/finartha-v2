...
def _px():
    ...
    try:
        ...
        # store diagnostic for optional display
        st.session_state["_plotly_missing_reason"] = str(e)
        return None

def create_spending_pie_chart(df: pd.DataFrame):
    """Return a Plotly Figure or None if Plotly isn't available."""
    px = _px()
    if px is None:
        st.error("Plotly is not installed. Add `plotly>=5.20` to requirements.txt and redeploy.")
        return None
    # adapt to your data shape as needed
...
def create_income_expense_bar_chart(df: pd.DataFrame):
    ...
    if px is None:
        st.error("Plotly is not installed. Add `plotly>=5.20` to requirements.txt and redeploy.")
        return None
    # Expecting columns: date, amount, type ∈ {"Income","Expense"} (adjust as needed)
    fig = px.bar(df, x="date", y="amount", color="type", barmode="group", title="Income vs Expense")
    fig.update_layout(xaxis_title="", yaxis_title="Amount")
    return fig

# Optional convenience wrapper so callers don't have to check for None every time.
def st_plotly_chart_safe(fig, **kwargs):
    """Render a figure if present; otherwise show a gentle placeholder."""
    if fig is None:
        st.info("Chart unavailable because Plotly isn't installed.")
        return
